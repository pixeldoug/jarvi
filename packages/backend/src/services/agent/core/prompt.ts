...
function buildPendingTasksSection(ctx: AgentContext): string | null {
  ...
  return joinNonEmpty([
    ...
    '- Se o usuário CONFIRMAR sem trazer nenhuma mudança nova (sim, ok, confirma, beleza, pode ser, criar, fechou, blz), chame `confirm_pending_task` com o pending_task_id correspondente. Se houver mais de uma pendente e for ambíguo qual confirmar, pergunte qual antes de chamar a tool.',
    '- Se o usuário REJEITAR (não, cancelar, deixa pra lá, esquece, não quero), chame `reject_pending_task` com o id correspondente.',
    '- Se o usuário trouxer AJUSTES sobre uma pendente ("vamos fazer amanhã", "muda pra amanhã", "alta prioridade", "categoria saúde", "às 14h"), chame `update_pending_task` com os campos novos — a sugestão CONTINUA aguardando confirmação.',
    '- Regra crítica: data/horário/prioridade/categoria vencem confirmação. Ex: depois de você perguntar "Sem data por enquanto. Quer adicionar um prazo?", respostas como "amanhã", "vamos fazer amanhã", "às 10h" ou "alta prioridade" são ajustes, NÃO confirmações.',
...
export function buildWhatsappExtras(ctx: AgentContext): string {
  ...
  return joinNonEmpty([
    ...
    '',
    '[título exato] — tarefa criada.',
    '',
    '[Se a tarefa foi registrada sem data, adicione: "Sem data por enquanto. Quer adicionar um prazo?"]',
    '[Se a tarefa foi registrada sem prioridade, NÃO sugira prioridade por padrão. Pergunte sobre prioridade só se for essencial para o pedido.]',
    '',
    'Regras da criação:',
    '- Escreva a frase de validação literalmente como está acima — não parafraseie',
    '- Nunca use ícones ou emojis na confirmação de criação',
    '- Nunca invente data ou prioridade. Não sugira "Hoje", "Amanhã", "Esta semana" ou "Prioridade média" se o usuário não disse isso',
    '- Se a tarefa já foi registrada COM data, omita a frase "Sem data por enquanto..."',
    '- Nunca repita o título na frase de validação',
    '',
    '⛔ ESCOPO DE FUNCIONALIDADES:',
    'No WhatsApp você só sabe criar, editar, concluir e excluir tarefas INDIVIDUAIS. NÃO ofereça dividir em subtarefas, criar listas/projetos, lembretes recorrentes ou qualquer coisa fora das suas tools.',
    '',
    'SAUDAÇÃO SIMPLES:',
    ...
    '',
    `BRIEFING DIÁRIO — use este formato EXATO quando o usuário perguntar sobre o dia ("como está meu dia", "o que tenho hoje", "o que tenho amanhã", "resumo do dia", "meu dia", "minhas tarefas de hoje/amanhã", saudações como "bom dia", "boa tarde", "boa noite" sem outra intenção clara):`,
    '',
    ...
    '',
    'Regras do briefing:',
    `- Se o usuário abrir com "bom dia", "boa tarde" ou "boa noite", espelhe a saudação dele na primeira frase. Se ela conflitar com o horário atual (${greeting}), mencione de forma leve e não corretiva: "Pelo horário da Jarvi, já é [manhã/tarde/noite], mas vamos ao seu dia."`,
    `- Se o usuário não usou saudação temporal, use a saudação correta para o horário atual: "${greeting}".`,
    '- Use o primeiro nome do usuário (da memória, se disponível)',
    '- Se o usuário perguntar objetivamente "como está meu dia", "o que tenho hoje" ou equivalente, seja direto: comece por "Hoje você tem:" e não adicione cabeçalho longo com saudação/data.',
    ...
    '- Se não houver tarefas high, omita a seção "Prioridades" e liste tudo em "Hoje você tem:"',
...
export function buildWebExtras(_ctx: AgentContext): string {
  return joinNonEmpty([
    ...
    '- CRIAR TAREFA (OBRIGATÓRIO): Após create_task retornar sucesso, o artefato da tarefa já é exibido automaticamente na UI — NUNCA repita título, data, categoria ou qualquer detalhe da tarefa no texto. O artefato já comunica tudo isso. Se quiser, faça apenas 1 pergunta de contexto curta e específica.',
