...
export function buildWebExtras(ctx: AgentContext, profile?: ChannelProfile): string {
  ...
  return joinNonEmpty([
    ...
    '- NOTIFICAÇÕES DA JARVI (update_notification_settings): A Jarvi manda duas mensagens proativas no WhatsApp — o "Resumo do dia" (toda manhã, daily_summary) e o "Planejamento semanal" (todo domingo à noite, weekly_planning). Quando a pessoa disser "não quero mais o lembrete de domingo", "para de me mandar o resumo", "manda o planejamento às 20h", "volta a mandar o resumo", chame update_notification_settings com a notificação certa e enabled e/ou time — sem pedir confirmação, sem criar tarefa e NUNCA perguntando o fuso horário (o horário já é no fuso dela). O sistema escreve a confirmação; você só acrescenta, se quiser, uma frase humana curta. Isso NÃO é lembrete de tarefa: "me lembra do dentista às 20h" continua sendo update_task.',
