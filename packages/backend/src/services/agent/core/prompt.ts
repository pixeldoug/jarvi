...
export function buildWebExtras(_ctx: AgentContext): string {
  return joinNonEmpty([
    ...
    '- DESCRIÇÃO ESTRUTURADA: Quando o usuário fornecer contexto rico (um anexo/imagem, um documento, ou vários detalhes), preencha o campo description de create_task OU update_task com uma descrição ORGANIZADA em Markdown — NÃO um parágrafo único. Use seções curtas com títulos `## ` (ex.: `## Contexto`, `## Objetivo`, `## Próximos passos`), listas com `- ` e checklists acionáveis com `- [ ] `. Sintetize o conteúdo do anexo (não transcreva tudo). Para pedidos triviais (ex.: "comprar pão"), mantenha a descrição curta e simples — só estruture quando há contexto suficiente para justificar. Esse Markdown é renderizado de forma formatada na tarefa.',
    ...
    '- ATUALIZAR CONTEXTO (CRÍTICO): Ao usar update_task para atualizar a descrição/contexto, você PODE reorganizar e reescrever o texto para incorporar informações novas de forma clara — mas é PROIBIDO omitir ou apagar informações relevantes que já estavam na tarefa. Inclua sempre o contexto anterior importante junto com as novidades. Use Markdown estruturado (## seções, listas, checklists), não um parágrafo único.',
...
export function buildTaskFocusedPrompt(
  ...
  return joinNonEmpty([
    ...
    '- ATUALIZAR CONTEXTO (CRÍTICO): Ao enriquecer a descrição desta tarefa, você PODE reescrever o texto para integrar o que é novo — mas é PROIBIDO omitir informações relevantes que já estavam salvas. Use Markdown estruturado (## seções, listas, checklists), não um parágrafo único.',
