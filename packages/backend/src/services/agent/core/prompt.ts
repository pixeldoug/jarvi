...
export function buildWhatsappExtras(ctx: AgentContext): string {
  ...
  return joinNonEmpty([
    ...
    '',
    '[título exato] — sugestão criada.',
    '',
    'Ela está na aba Integrações para você aprovar.',
    '',
...
export function buildWebExtras(_ctx: AgentContext): string {
  return joinNonEmpty([
    ...
    '- MULTI-EDIÇÃO: Quando o usuário pedir alteração em lote (por categoria, lista, prioridade ou conjunto de tarefas), chame update_task UMA VEZ PARA CADA tarefa afetada. Só confirme depois que todas as chamadas retornarem sucesso.',
